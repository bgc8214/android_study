...
//      TODO (35) Initialize the loader for DetailActivity
    ...
    public void onLoadFinished(@NonNull Loader<Cursor> loader, Cursor cursor) {
        ...
        long localDateMidnightGmt = cursor.getLong(DATE);
        String dateText = SunshineDateUtils.getFriendlyDateString(this, localDateMidnightGmt, true);

        mDateView.setText(dateText);

        int weatherId = cursor.getInt(CONDITION_ID);
        String description = SunshineWeatherUtils.getStringForWeatherCondition(this, weatherId);

        mDescriptionView.setText(description);

        double highInCelsius = cursor.getDouble(MAX_TEMP);
        ...
        double lowInCelsius = cursor.getDouble(MIN_TEMP);

        String lowString = SunshineWeatherUtils.formatTemperature(this, lowInCelsius);

        mLowTemperatureView.setText(lowString);

        float humidity = cursor.getFloat(HUMIDITY);
        String humidityString = getString(R.string.format_humidity, humidity);


        mHumidityView.setText(humidityString);

        float windSpeed = cursor.getFloat(WIND_SPEED);
        float windDirection = cursor.getFloat(DEGREES);
        String windString = SunshineWeatherUtils.getFormattedWind(this, windSpeed, windDirection);


        mWindView.setText(windString);

        float pressure = cursor.getFloat(PRESSURE);
